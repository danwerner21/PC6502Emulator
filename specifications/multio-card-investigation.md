...
1. **Fix the self-test port mismatch** (§6.2): real firmware writes `$AA` to `$E3FF` (offset 1), not `$E3FE` (offset 0). Whether to fix the emulator's offset check or explicitly decide the stub's current behavior is intentional (and if so, document why) is an implementation/product decision for whoever picks this up — this document only establishes that the two disagree.
   **Fixed by mc-zrr** (commit `d2a5fe4`, 2026-07-03): `multiio_write` now checks offset 1, matching `KBD_PUTCMD`'s `STA KBD_CMD`.
2. **Model the status register (`$E3FF`) for real.** Bits 0 (output-data-pending) and 1 (input-buffer-busy) currently always return raw `open_bus`, never reflecting `kbd_selftest_pending` or any other internal state. Firmware's `KBD_PUTCMD`/`KBD_PUTDATA`/`KBD_GETDATA` polling loops (§3.2) are not genuinely modeled today — they merely happen to proceed or stall depending on the configured `open_bus` byte's bit pattern.
   **Fixed by mc-hpg** (2026-07-03): this was the actual cause of the human-reported boot-time timeout, confirmed via real-firmware execution. With the mc-zrr offset fix alone, real firmware still never reached the corrected `$E3FF` write: `KBD_PUTCMD`'s own pre-write busy-wait (`bios_multi.asm:269-283`, polls bit 1) read the default open-bus byte (`$EA`, bit 1 = 1 = "busy") forever and timed out before ever executing `STA KBD_CMD`, producing `KBD: VT82C42 WRITE TIMEOUT.` on every real boot. `multiio_read` now forces bit 1 permanently clear (writes are processed synchronously, so the controller is never busy) and derives bit 0 from `kbd_selftest_pending` (set when `$AA` is written, cleared when `$E3FE` is read); other status bits still mirror `open_bus`. Verified via `emulator/examples/boot_diag.rs` against the real `rom.hex`/`disk.img`: the boot transcript now reads `KBD: INITIALIZED.` and never contains `VT82C42`, and DOS/65 reaches `A>`. New gate coverage: `emulator/tests/m6_rtc_config_gate.rs::multiio_status_never_reports_busy_even_when_open_bus_bit1_is_set`, `::multiio_status_data_pending_tracks_selftest_state`, and a strengthened `emulator/tests/m4_dos_boot_gate.rs::m4_real_boot_far_call_and_sim_init` that asserts the real-firmware boot transcript never contains `VT82C42`.
