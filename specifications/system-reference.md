...
- **Two emulator bugs found in the RTC model** (§10.4): a register-offset shift that swaps weekday into the day/month/year field range, and a STOP-bit check against the wrong bit. Both are in `emulator/src/rtc.rs`; the M6 gate test asserts the buggy offsets as correct, so `cargo test` passing does not catch this. **Fixed 2026-07-03 (bead `mc-bkp`, commit `f214a3c`)** — see §10.4 for the corrected register map.
- **Four config knobs created for P0/P1 unknowns are declared but never consulted** by the emulation logic (`cpu_subtype`, `cpu_hz`, `shadow_addr_low`, `acia_variant`; see §9 status column for each). They round-trip through TOML and are covered by `tests/config_load.rs`, which can give a false impression that the underlying unknown is handled. `io_rom_always` was in this list too; **fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`)** — see R0.7 in §9.
- **`$EFE6` and `$EFE7` (MMU) are not implemented at all** — both fall through to the generic open-bus arm rather than their documented specific behavior. This is a stronger claim than "unknown": the emulator has not yet made *any* choice here. **Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`)** — see R1.5/R1.6 in §9.
...
| R0.5 | Open-bus read value for unmapped addresses | Reads to `$EFA0-$EFCF`, `$EFF0-$EFFF`, unassigned MMU offsets, and absent physical pages return unknown data. Must not silently return `$00`; should use a named, configurable policy. → MEM §9.4, §13, HW §7.3 | **Resolved (tooling).** `open_bus.value` (default `0xEA`) is threaded through `Bus`, `Mmu`, `XtIde`, `Peripherals` and covered by gate test `open_bus_at_efa0`. **Updated 2026-07-03 (bead `pe-xr4`, commit `3045169`):** `$EFE6` still returns this value, but now deliberately (it's the documented "no useful data defined" byte, paired with an observable TC-counter side effect); `$EFE7` no longer falls into this arm at all — it returns a deliberate `$0E` — see R1.5/R1.6. |
...
| R0.7 | ROM/I/O overlay precedence over arbitrary MMU mappings | It is confirmed that task-0 logical pages `$E` and `$F` identity-map to physical I/O and ROM, and that this works correctly. Whether the board always forces I/O/ROM decode regardless of the MMU mapping at those physical pages is **unknown**. Only the identity-map case is safe to assume. → MEM §3.2, ROM §7.2, §7.3 | **Resolved (tooling). Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** `io_rom_always` now gates a physical-page decode path in `Bus::read`/`write` (`bus.rs`): when true and the MMU is enabled, every address is translated first and dispatched by the resulting physical page (`$0E`→I/O, `$0F`→ROM, else RAM), so a task remapping a low logical page onto physical `$0E`/`$0F` reaches I/O/ROM instead of aliasing into RAM. Default remains `false`, which preserves the exact prior CPU-address-range decode (the one case confirmed on hardware) — the "only identity-map case is safe" caveat still applies to the default, but the alternative is no longer dead code. Covered by `io_rom_always_relocates_io_and_rom` and the `io_rom_always_default_false_does_not_relocate` regression test. |
...
| R1.5 | `$EFE6` terminal-count pulse width, polarity, and clearing | Firmware reads it in the IDE driver path; effect on real hardware is not established. → MEM §5.2, IO §CPU-visible decode | **Observable event modeled. Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** `Mmu::io_read` offset `0x16` now increments a `tc_signal_count` counter on every read (`mmu.rs`), giving tests something to observe without inventing pulse timing or a returned data value — the byte itself stays open-bus, matching "no useful returned data is defined." Pulse width/polarity/clearing remain unmodeled, consistent with "defer cycle-accurate pulse timing" (MEM §13). Covered by `efe6_read_asserts_observable_tc_signal`. |
| R1.6 | `$EFE7` I/O-page readback under nondefault shadow straps | The registered value is confirmed; semantics under alternate configurations are not. → MEM §5.2, §13, MEM §15 | **Resolved for the default strap. Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** `Mmu::io_read` offset `0x17` now returns bits `3:0` = `$0E` (the firmware-compatible I/O physical page), matching MEM §4.2/§13's "return the documented default page value" guidance; upper bits are 0 (spec: "not meaningful"). Nondefault shadow straps are still unmodeled — no code implements an alternate strap at all (see R0.6). Covered by `efe7_reports_io_physical_page_low_nibble`. |
| R1.7 | MMU active-task alias defect — which board revisions have it | Without knowing revision, implementing as off-by-default is the only safe choice. → MEM §8, §13 | **Named optional mode now exists. Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** New `mmu_task0_alias_defect` config flag (default `false`) gates a second `map` write in `Mmu::io_write`'s edit-window arm that duplicates every `$EFD0-$EFDF` write into task 0 regardless of the selected setup task. Off by default, matching "the clean architectural model"; the compatibility mode is opt-in exactly as MEM §8/§11 item 12 calls for. Covered by `task0_alias_defect_duplicates_edit_writes_when_enabled` and the off-by-default regression test. |
...
- [x] Give physical I/O overlay (page `$0E`) and ROM overlay (page `$0F`) priority over hidden SRAM for read and write. — **True for the default; selectable for the general case. Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** Default (`io_rom_always=false`) keeps the CPU-address-range decode ahead of any MMU lookup — the one case confirmed on hardware. Setting `io_rom_always=true` makes `Bus::read`/`write` decode every address by its *physical* page after translation, so a task that remaps a low logical page to physical `$0E`/`$0F` reaches I/O/ROM instead of RAM. See R0.7 in §9.
- [x] ROM writes must not update hidden SRAM. — `0xF000..=0xFFFF => {}` is a hard no-op in `Bus::write` (`bus.rs:74`).
- [x] Unclaimed physical reads use a named, configurable open-bus policy (not silently `$00`); unclaimed writes are discarded. — `open_bus.value` (default `0xEA`), threaded through `Bus`/`Mmu`/`XtIde`/`Peripherals`. **Updated 2026-07-03 (bead `pe-xr4`, commit `3045169`):** `$EFE6` reaches this value by deliberate policy now (dedicated match arm with a TC-counter side effect); `$EFE7` no longer reaches it at all — see §10.2.
...
- [x] `$EFE6` read: produce an observable TC event; do not depend on read data value. — **Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** Offset `0x16` increments a `tc_signal_count` counter on each read; the returned byte stays open-bus (undefined per spec). See R1.5 in §9.
- [x] `$EFE7` read: return documented I/O-page default in low 4 bits; document upper bits. — **Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** Offset `0x17` returns `$0E` (bits 3:0 = I/O physical page; upper bits 0, documented as not meaningful). See R1.6 in §9.
- [x] Unassigned offsets (`$EFE3`, `$EFE5`, `$EFE8-$EFEF`): use open-bus policy; writes are no-ops. — Falls through to `_ => self.open_bus` (read) / `_ => {}` (write). **Updated 2026-07-03 (bead `pe-xr4`, commit `3045169`):** `$EFE6`/`$EFE7` now have their own dedicated match arms (see the two rows above) and no longer share this fallthrough.
...
- [x] Task-0 alias defect: default off; implement as named optional mode. — **Fixed 2026-07-03 (bead `pe-xr4`, commit `3045169`).** `mmu_task0_alias_defect` config flag (default `false`) gates a second write into task 0's map entry on every edit-window write. See R1.7 in §9. → MEM §11, §12
...
- [x] **RTC:** implement RTC-72421/72423 register model; low nibble only for data registers. — **Fixed 2026-07-03 (bead `mc-bkp`, commit `f214a3c`).** Was: register offsets shifted starting at `$EF96`, verified against firmware ground truth `bios_rtc.asm`'s `RTC_READ`/`RTC_WRITE` (`PC6502_firmware_source/bios_rtc.asm:22-93`), which address raw offset `2×field` with logical field 6 explicitly decremented to field 5 (`CPX #6 / BNE / DEX`) — giving raw layout `$0-1`=Sec, `$2-3`=Min, `$4-5`=Hour, `$6-7`=Day, `$8-9`=Month, `$A-B`=Year (fields 5 and 6 alias), `$C`=Weekday (never addressed by the driver, but still part of the chip's register model). `Rtc::populate_from_unix()` (`rtc.rs:58-70`) now writes Day/Month/Year starting at `0x06` (one register earlier than before) and weekday last, at `0x0C`, matching this layout and §6.2's raw register table. `m6_rtc_config_gate.rs`'s `rtc_fixed_matches_epoch` and `rtc_host_year_plausible` are updated to assert the corrected offsets; full M1–M6 gate suite re-run clean (13/13 non-ignored, 1 ignored `m5_com_load`, unrelated).
- [x] **RTC:** reproduce the exact control-register write sequence (`$02`/digits/`$00`/`$00`/`$01`/`$05`/`$04`) with correct side effects. — **Fixed 2026-07-03 (bead `mc-bkp`, commit `f214a3c`).** Was: STOP checked against bit 3 (TEST) instead of bit 1 (STOP) per §6.2's own register table. `Rtc::write()` (`rtc.rs:41-43`) now sets `self.stopped` from `val & 0x02`. Practical effect remains small since `read()` recomputes from the clock policy on every access rather than freezing a ticking register set, but the control sequence now sets/clears the correct bit.
...
- [x] **RTC:** weekday register `$C` is never set by PC6502 firmware; reflect this in test coverage. — Still true of the *firmware*: `bios_rtc.asm` never addresses raw offset `$C`. **Fixed 2026-07-03 (bead `mc-bkp`, commit `f214a3c`)** on the *emulator* side: as a side effect of the offset-shift bug above, `rtc.rs` used to write year-tens data into offset `0x0C` on every clock update; it now writes the correct weekday value there instead, completing the RTC-72421/72423 register model (§6.2) even though this firmware happens not to read that offset.
...
- [x] **[2026-07-02, new item]** The shipped `emulator/disk_image/disk.img`'s CP/M directory is at LBA `0x0100` (256), not LBA 60, and is populated: 29 active files, 24 of them `.COM`. See §7.2 for the full file list and provenance. **Do not assume an empty disk when testing or implementing DOS/65 file operations against the stock image.** — This correction (from `plans/pc6502-emulator-milestones/artifact-verification.md` at commit `f48cfbe`) unblocked REQ-M5-1 (`DIR A:`) and REQ-M5-2 (`.COM` load) gate-test work: `m5_dir_listing` now exists and passes (REQ-M5-1 implemented), and **REQ-M5-2 is now also implemented** — **Fixed 2026-07-03 (bead `mc-01g`, commit `f57cddf`).** `m5_com_load` in `emulator/tests/m5_disk_io_gate.rs` is no longer `#[ignore]`d. It invokes `S19.COM` (bare `S19`, per DOS/65's CCM rejecting an explicit type on transient commands) and feeds its ROM-resident Motorola-S-record loader (`JSR $FFF3`) an `"S9"` terminator, which the loader RTS's on immediately — proving the `.COM` loads and runs without needing a real S-record payload. The terminator is injected only after execution reaches `$0800` (the TEA), not queued upfront with the command line: queuing it upfront let a stray byte get consumed during the disk-load sequence before the loader started reading, hanging it indefinitely — this was a test-harness timing bug, not an emulator bug. Full M1–M6 gate suite re-run clean: 21/21 pass, 0 ignored.
